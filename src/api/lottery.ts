...
// 获取分组配置
export async function fetchGroupConfig(themeId: string): Promise<any> {
  try {
    const res = await api.get(`/themes/${themeId}/group`)
    return res.data.data
  }
  catch {
    return null
...
// 保存分组配置
export async function saveGroupConfig(themeId: string, data: any): Promise<void> {
  try {
    await api.post(`/themes/${themeId}/group`, data)
  }
  catch (error) {
