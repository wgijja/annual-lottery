...
function initTableData() {
  ...
  const isTemporaryPrize = temporaryPrize.value && 
                           temporaryPrize.value.isShow && 
                           temporaryPrize.value.id === currentPrize.value?.id
  
  // 如果是临时奖项，渲染全部人；否则根据groupIds过滤
  if (!isTemporaryPrize && currentPrize.value) {
...
function transform(targets: any[], duration: number) {
  ...
  return new Promise((resolve) => {
    ...
    // 如果是转换到球型，先重置场景旋转
    const isSphereTransform = targets === targets.sphere
    if (isSphereTransform) {
...
function startLottery() {
  ...
  // 获取基础人员池（从全部人员开始，根据当前奖项配置过滤）
  let basePersonList = allPersonList.value
  
  // 根据分组和eligible过滤人员池
  personPool.value = basePersonList.filter((person: IPersonConfig) => {
    // 1. 过滤eligible：eligible为false的人员不能中奖
    if (person.eligible === false) {
      return false
    }
    
    // 2. 过滤分组：如果奖项指定了groupIds，只从指定组中抽取
    const prizeGroupIds = currentPrize.value.groupIds
    if (prizeGroupIds && prizeGroupIds.length > 0) {
      // 如果人员没有分组或不在指定组中，则排除
      if (!person.groupId || !prizeGroupIds.includes(person.groupId)) {
        return false
      }
    }
    
    // 3. 根据isAll决定是否排除已中此奖的人员
    if (currentPrize.value.isAll) {
      // isAll为true：排除已中此奖的人员（但可以中其他奖）
      if (person.prizeId && person.prizeId.includes(currentPrize.value.id as string)) {
        return false
      }
    } else {
      // isAll为false：只从未中奖的人员中抽取（不能中任何奖）
      if (person.isWin === true) {
        return false
      }
    }
  ...
  // 使用frequency作为单次抽取个数，如果没有设置则默认为10
  const defaultFrequency = currentPrize.value.frequency || 10
  luckyCount.value = defaultFrequency
  
  // 自定义抽奖个数（如果启用了separateCount）
  ...
  // 确保抽取个数不超过剩余数量和设置的frequency
  luckyCount.value = Math.min(leftover, luckyCount.value)
...
watch(
  ...
  () => {
    ...
    if (!isMobile.value && tableData.value.length > 0) {
      reinitCanvas()
    }
  },
  { deep: true }
...
<template>
  ...
  <div class="header-area">
    ...
    <div v-if="tableData.length <= 0" class="empty-actions">
      <button class="action-btn primary" @click="goToConfig">
